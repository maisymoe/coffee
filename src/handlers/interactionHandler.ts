...
export default async (client: CoffeeBot) => {
    ...
    client.on("interactionCreate", async (interaction: Interaction) => {
        ...
        const command: Command | undefined = client.registry.commands.find(
            (i) => i.name === interaction.commandName,
        );
            ...
            return interaction.reply({
                embeds: [
                    await createErrorEmbed(
                        ...
                        } is invalid. Valid commands are ${client.registry.commands.map((c) => {
                            return c.name;
                        })}`,
                    ),
                ],
        ...
        if (command!.su && !client.dynamicData.config.data.users!.includes(interaction.user.id)) {
            await interaction.editReply({
                embeds: [
                    await createErrorEmbed(
                        "Missing permissions",
                        `${interaction.user.username} is not in the sudoers file. This incident will be reported.`,
                    ),
                ],
            });
            return;
        ...
        try {
            ...
            await interaction.editReply({
                embeds: [
                    await createErrorEmbed(
                        "Interaction handler threw an exception...",

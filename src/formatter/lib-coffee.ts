...
export function installCoffee(vm: VM, context: VMContext): void {
    vm.install({
        "frien": wrapFunc("frien", 2, async (args: Value[]): Promise<Value> => {
            ...
            if (member1 && member2) return `${member1.nickname || member1.user.username} is friens with ${member2.nickname || member2.user.username}`;
        ...
        "frien-list": wrapFunc("frien-list", 1, async (args: Value[]): Promise<Value> => {
            ...
            if (listOfFriens.length > 0) return `These people are all friends: ${listOfFriens.map(m => { 
